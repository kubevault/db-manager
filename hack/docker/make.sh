...
DIST=$GOPATH/src/github.com/kubevault/db-manager/dist
mkdir -p $DIST
if [ -f "$DIST/.tag" ]; then
...
clean() {
  pushd $GOPATH/src/github.com/kubevault/db-manager/hack/docker
  rm messenger Dockerfile
  popd
}

build_binary() {
  pushd $GOPATH/src/github.com/kubevault/db-manager
  ./hack/builddeps.sh
